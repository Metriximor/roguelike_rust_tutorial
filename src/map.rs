    ...
    for _ in 0..MAX_ROOMS {
        ...
        if ok {
            ...
            if !rooms.is_empty() {
                ...
                if rng.range(0, 2) == 1 {
                    apply_corridor_horizontal_to_map(&mut map, prev_x, new_x, prev_y);
                    apply_corridor_vertical_to_map(&mut map, prev_y, new_y, new_x);
                } else {
                    apply_corridor_vertical_to_map(&mut map, prev_y, new_y, prev_x);
                    apply_corridor_horizontal_to_map(&mut map, prev_x, new_x, new_y);
                }
            }
            rooms.push(new_room);
        }
...
fn apply_corridor_horizontal_to_map(map: &mut [TileType], x1: i32, x2: i32, y: i32) {
...
fn apply_corridor_vertical_to_map(map: &mut [TileType], y1: i32, y2: i32, x: i32) {
